...
class BouncingBall {
  ...
  // Detects whether the ball is colliding with the left wall.
  boolean collidesLeft(Posn topLeft) {
    return Math.abs(pos.x - topLeft.x) < this.size && this.dx < 0;
  }

  // Detects whether the ball is colliding with the right wall.
  boolean collidesRight(Posn botRight) {
    return Math.abs(botRight.x - this.pos.x) < this.size && this.dx > 0;
  }

  // Detects whether the ball is colliding with a side wall.
  boolean collidesX(Posn topLeft, Posn botRight) {
    return collidesLeft(topLeft) || collidesRight(botRight);
  }

  // Detects whether the ball is colliding with the top wall.
  boolean collidesTop(Posn topLeft) {
    return Math.abs(pos.y - topLeft.y) < this.size && this.dy < 0;
  }

  // Detects whether the ball is colliding with the bottom wall.
  boolean collidesBottom(Posn botRight) {
    return Math.abs(botRight.y - this.pos.y) < this.size && this.dy > 0;
  }

  // Detects whether the ball is colliding with a top or bottom wall.
  boolean collidesY(Posn topLeft, Posn botRight) {
    return collidesTop(topLeft) || collidesBottom(botRight);
  }
...
class ExamplesBouncingBalls {
  ...
  BouncingBall b4 = new BouncingBall(new Posn(0, 0), Color.RED, 5, -1, -1);
  ...
  boolean testBouncingBall(Tester t) {
    return t.checkInexact(b.area(), 78.5, EPS)
        ...
        && t.checkExpect(b.collidesX(new Posn(-2, -10), new Posn(10, 10)), false)
        && t.checkExpect(b.collidesX(new Posn(-10, -10), new Posn(2, 10)), true)
        && t.checkExpect(b.collidesX(new Posn(-10, -10), new Posn(10, 2)), false)
        && t.checkExpect(b.collidesY(new Posn(-10, -2), new Posn(10, 10)), false)
        && t.checkExpect(b.collidesY(new Posn(-10, -10), new Posn(10, 2)), true)
        && t.checkExpect(b.collidesY(new Posn(-10, -10), new Posn(2, 10)), false)
        && t.checkExpect(b.collidesBottom(new Posn(0, 10)), false)
        && t.checkExpect(b4.collidesBottom(new Posn(0, 2)), false)
        && t.checkExpect(b.collidesBottom(new Posn(0, 2)), true)
        && t.checkExpect(b4.collidesTop(new Posn(0, -10)), false)
        && t.checkExpect(b.collidesTop(new Posn(0, -2)), false)
        && t.checkExpect(b4.collidesTop(new Posn(0, -2)), true)
        && t.checkExpect(b4.collidesLeft(new Posn(-10, 0)), false)
        && t.checkExpect(b.collidesLeft(new Posn(-2, 0)), false)
        && t.checkExpect(b4.collidesLeft(new Posn(-2, 0)), true)
        && t.checkExpect(b.collidesRight(new Posn(10, 0)), false)
        && t.checkExpect(b4.collidesRight(new Posn(2, 0)), false)
        && t.checkExpect(b.collidesRight(new Posn(2, 0)), true);
  }
}